...
namespace OblivionModManager.Forms {
    internal partial class DataFileBrowser : Form {
        [Flags]
        private enum TreeNodeType { Folder=1, BSA=2, Unparented=4, Parented=8, Any=0xFF }
        internal class TreeViewSorter : System.Collections.IComparer {
        ...
        internal DataFileBrowser(bool ShowBSAs) {
            InitializeComponent();
            BSAs=ShowBSAs;
            BuildTreeView();
        }
        ...
        private void BuildTreeView() {
            ...
            if(BSAs) {
                foreach(string file in Directory.GetFiles("Data", "*.bsa")) {
