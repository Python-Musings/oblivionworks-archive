    ...
    internal static class obmmScriptHandler {
        ...
        private static bool FunctionIf(string[] line) {
            ...
            case "DataFileExists":
                ...
                return File.Exists("data\\"+line[2]);
            ...
            case "ScriptExtenderPresent":
                ...
                return File.Exists("obse_loader.exe");
            case "ScriptExtenderNewerThan":
                ...
                if(!File.Exists("obse_loader.exe")) return false;
                try {
                    System.Diagnostics.FileVersionInfo fvi=System.Diagnostics.FileVersionInfo.GetVersionInfo("obse_loader.exe");
            ...
            case "GraphicsExtenderPresent":
                ...
                return File.Exists(@"data\obse\plugins\obge.dll");
            case "GraphicsExtenderNewerThan":
                ...
                if(!File.Exists(@"data\obse\plugins\obge.dll")) return false;
                try {
                    System.Diagnostics.FileVersionInfo fvi=System.Diagnostics.FileVersionInfo.GetVersionInfo(@"data\obse\plugins\obge.dll");
                    if(fvi.FileVersion==null) return false;
                    Version v=new Version(line[2]); ;
                    Version v2=new Version(fvi.FileVersion.Replace(", ", "."));
                    return (v2>=v);
        ...
        private static void FunctionPatch(string[] line, bool Plugin) {
            ...
            if(File.Exists("Data\\"+line[2])) {
                ...
                timestamp=File.GetLastWriteTime("Data\\"+line[2]);
                File.Delete("Data\\"+line[2]);
            } else if(line.Length<4||line[3]!="True") return;
            File.Move(copypath, "Data\\"+line[2]);
            File.SetLastWriteTime("Data\\"+line[2], timestamp);
        }
        ...
        private static void FunctionSetEspData(string[] line, Type type) {
            ...
            System.IO.FileStream fs=File.OpenWrite(Plugins+line[1]);
