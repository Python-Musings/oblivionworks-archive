...
def initDirs():
    ...
    if mashIni:
        if mashIni.has_option('Tool Options','sTes4FilesPath'):
            dirs['TES4FilesPath'] = GPath(mashIni.get('Tool Options','sTes4FilesPath').strip())
            if not dirs['TES4FilesPath'].isabs():
                dirs['TES4FilesPath'] = dirs['app'].join(dirs['TES4FilesPath'])
        if mashIni.has_option('Tool Options','sTes4EditPath'):
            dirs['TES4EditPath'] = GPath(mashIni.get('Tool Options','sTes4EditPath').strip())
            if not dirs['TES4EditPath'].isabs():
                dirs['TES4EditPath'] = dirs['app'].join(dirs['TES4EditPath'])
        if mashIni.has_option('Tool Options','sTes4LodGenPath'):
            dirs['TES4LodGenPath'] = GPath(mashIni.get('Tool Options','sTes4LodGenPath').strip())
            if not dirs['TES4LodGenPath'].isabs():
                dirs['TES4LodGenPath'] = dirs['app'].join(dirs['TES4LodGenPath'])
        if mashIni.has_option('Tool Options','sNifskopePath'):
            dirs['NifskopePath'] = GPath(mashIni.get('Tool Options','sNifskopePath').strip())
            if not dirs['NifskopePath'].isabs():
                dirs['NifskopePath'] = dirs['app'].join(dirs['NifskopePath'])
        if mashIni.has_option('Tool Options','sBlenderPath'):
            dirs['BlenderPath'] = GPath(mashIni.get('Tool Options','sBlenderPath').strip())
            if not dirs['BlenderPath'].isabs():
                dirs['BlenderPath'] = dirs['app'].join(dirs['BlenderPath'])
        if mashIni.has_option('Tool Options','sGmaxPath'):
            dirs['GmaxPath'] = GPath(mashIni.get('Tool Options','sGmaxPath').strip())
            if not dirs['GmaxPath'].isabs():
                dirs['GmaxPath'] = dirs['app'].join(dirs['GmaxPath'])
        if mashIni.has_option('Tool Options','sMaxPath'):
            dirs['MaxPath'] = GPath(mashIni.get('Tool Options','sMaxPath').strip())
            if not dirs['MaxPath'].isabs():
                dirs['MaxPath'] = dirs['app'].join(dirs['MaxPath'])
        if mashIni.has_option('Tool Options','sMayaPath'):
            dirs['MayaPath'] = GPath(mashIni.get('Tool Options','sMayaPath').strip())
            if not dirs['MayaPath'].isabs():
                dirs['MayaPath'] = dirs['app'].join(dirs['MayaPath'])
        if mashIni.has_option('Tool Options','sPhotoshopPath'):
            dirs['Photoshop'] = GPath(mashIni.get('Tool Options','sPhotoshopPath').strip())
            if not dirs['Photoshop'].isabs():
                dirs['Photoshop'] = dirs['app'].join(dirs['Photoshop'])
        if mashIni.has_option('Tool Options','sGIMP'):
            dirs['GIMP'] = GPath(mashIni.get('Tool Options','sGIMP').strip())
            if not dirs['GIMP'].isabs():
                dirs['GIMP'] = dirs['app'].join(dirs['GIMP'])
        if mashIni.has_option('Tool Options','sISOBL'):
            dirs['ISOBL'] = GPath(mashIni.get('Tool Options','sISOBL').strip())
            if not dirs['ISOBL'].isabs():
                dirs['ISOBL'] = dirs['app'].join(dirs['ISOBL'])
        if mashIni.has_option('Tool Options','sISRMG'):
            dirs['ISRMG'] = GPath(mashIni.get('Tool Options','sISRMG').strip())
            if not dirs['ISRMG'].isabs():
                dirs['ISRMG'] = dirs['app'].join(dirs['ISRMG'])
        if mashIni.has_option('Tool Options','sISRNG'):
            dirs['ISRNG'] = GPath(mashIni.get('Tool Options','sISRNG').strip())
            if not dirs['ISRNG'].isabs():
                dirs['ISRNG'] = dirs['app'].join(dirs['ISRNG'])
        if mashIni.has_option('Tool Options','sISRNPCG'):
            dirs['ISRNPCG'] = GPath(mashIni.get('Tool Options','sISRNPCG').strip())
            if not dirs['ISRNPCG'].isabs():
                dirs['ISRNPCG'] = dirs['app'].join(dirs['ISRNPCG'])
        if mashIni.has_option('Tool Options','sNPP'):
            dirs['NPP'] = GPath(mashIni.get('Tool Options','sNPP').strip())
            if not dirs['NPP'].isabs():
                dirs['NPP'] = dirs['app'].join(dirs['NPP'])
        if mashIni.has_option('Tool Options','sFraps'):
            dirs['Fraps'] = GPath(mashIni.get('Tool Options','sFraps').strip())
            if not dirs['Fraps'].isabs():
                dirs['Fraps'] = dirs['app'].join(dirs['Fraps'])
        if mashIni.has_option('Tool Options','sAudacity'):
            dirs['Audacity'] = GPath(mashIni.get('Tool Options','sAudacity').strip())
            if not dirs['Audacity'].isabs():
                dirs['Audacity'] = dirs['app'].join(dirs['Audacity'])
        if mashIni.has_option('Tool Options','sArtweaver'):
            dirs['Artweaver'] = GPath(mashIni.get('Tool Options','sArtweaver').strip())
            if not dirs['Artweaver'].isabs():
                dirs['Artweaver'] = dirs['app'].join(dirs['Artweaver'])
        if mashIni.has_option('Tool Options','sDDSConverter'):
            dirs['DDSConverter'] = GPath(mashIni.get('Tool Options','sDDSConverter').strip())
            if not dirs['DDSConverter'].isabs():
                dirs['DDSConverter'] = dirs['app'].join(dirs['DDSConverter'])
        if mashIni.has_option('Tool Options','sCustom1'):
            dirs['Custom1'] = GPath(mashIni.get('Tool Options','sCustom1').strip())
            if not dirs['Custom1'].isabs():
                dirs['Custom1'] = dirs['app'].join(dirs['Custom1'])
        if mashIni.has_option('Tool Options','sCustom2'):
            dirs['Custom2'] = GPath(mashIni.get('Tool Options','sCustom2').strip())
            if not dirs['Custom2'].isabs():
                dirs['Custom2'] = dirs['app'].join(dirs['Custom2'])
        if mashIni.has_option('Tool Options','sCustom3'):
            dirs['Custom3'] = GPath(mashIni.get('Tool Options','sCustom3').strip())
            if not dirs['Custom3'].isabs():
                dirs['Custom3'] = dirs['app'].join(dirs['Custom3'])
        if mashIni.has_option('Tool Options','sCustom4'):
            dirs['Custom4'] = GPath(mashIni.get('Tool Options','sCustom4').strip())
            if not dirs['Custom4'].isabs():
                dirs['Custom4'] = dirs['app'].join(dirs['Custom4'])
        if mashIni.has_option('Tool Options','sPaintNET'):
            dirs['PaintNET'] = GPath(mashIni.get('Tool Options','sPaintNET').strip())
    ...
    if mashIni:
        if mashIni.has_option('Settings','sScriptFileExt'):
            inisettings['scriptFileExt'] = str(mashIni.get('Settings','sScriptFileExt').strip())
        if mashIni.has_option('Settings','iKeepLog'):
            inisettings['keepLog'] = int(mashIni.get('Settings','iKeepLog').strip())
        if mashIni.has_option('Settings','iIconSize'):
            inisettings['iconSize'] = str(mashIni.get('Settings','iIconSize').strip())
        if mashIni.has_option('Settings','sLogFile'):
            inisettings['logFile'] = GPath(mashIni.get('Settings','sLogFile').strip())
            if not inisettings['logFile'].isabs():
                inisettings['logFile'] = dirs['app'].join(inisettings['logFile'])
        if mashIni.has_option('Settings','bEnableWizard'):
            inisettings['enablewizard'] = int(mashIni.get('Settings','bEnableWizard').strip())
        if mashIni.has_option('Settings','bAutoItemCheck') or mashIni.has_option('Settings','sAutoItemCheck'): #Check "s..." for backwards compatibility
            if mashIni.has_option('Settings','bAutoItemCheck'): inisettings['AutoItemCheck'] = mashIni.getboolean('Settings','bAutoItemCheck')
            else: inisettings['AutoItemCheck'] = mashIni.getboolean('Settings','sAutoItemCheck')
        if mashIni.has_option('Tool Options','bshowtexturetoollaunchers'):
            inisettings['showtexturetoollaunchers'] = mashIni.getboolean('Tool Options','bshowtexturetoollaunchers')
        if mashIni.has_option('Tool Options','bshowmodelingtoollaunchers'):
            inisettings['showmodelingtoollaunchers'] = mashIni.getboolean('Tool Options','bshowmodelingtoollaunchers')
        if mashIni.has_option('Tool Options','bshowaudiotoollaunchers'):
            inisettings['showaudiotoollaunchers'] = mashIni.getboolean('Tool Options','bshowaudiotoollaunchers')
        if mashIni.has_option('Tool Options','bShowTes4View'):
            inisettings['ShowTes4View'] = mashIni.getboolean('Tool Options','bShowTes4View')
        if mashIni.has_option('Tool Options','bShowTes4Edit'):
            inisettings['ShowTes4Edit'] = mashIni.getboolean('Tool Options','bShowTes4Edit')
        if mashIni.has_option('Tool Options','bShowTes4Trans'):
            inisettings['ShowTes4Trans'] = mashIni.getboolean('Tool Options','bShowTes4Trans')
        if mashIni.has_option('Tool Options','sCustom1txt'):
            inisettings['custom1txt'] = str(mashIni.get('Tool Options','sCustom1txt')).strip()
        if mashIni.has_option('Tool Options','sCustom2txt'):
            inisettings['custom2txt'] = str(mashIni.get('Tool Options','sCustom2txt')).strip()
        if mashIni.has_option('Tool Options','sCustom3txt'):
            inisettings['custom3txt'] = str(mashIni.get('Tool Options','sCustom3txt')).strip()
        if mashIni.has_option('Tool Options','sCustom4txt'):
            inisettings['custom4txt'] = str(mashIni.get('Tool Options','sCustom4txt')).strip()
        if mashIni.has_option('Tool Options','sCustom1opt'):
            if not str(mashIni.get('Tool Options','sCustom1opt')).strip() == '-Option(s)':
                inisettings['custom1opt'] = str(mashIni.get('Tool Options','sCustom1opt')).strip()
        if mashIni.has_option('Tool Options','sCustom2opt'):
            if not str(mashIni.get('Tool Options','sCustom2opt')).strip() == '-Option(s)':
                inisettings['custom2opt'] = str(mashIni.get('Tool Options','sCustom2opt')).strip()
        if mashIni.has_option('Tool Options','sCustom3opt'):
            if not str(mashIni.get('Tool Options','sCustom3opt')).strip() == '-Option(s)':
                inisettings['custom3opt'] = str(mashIni.get('Tool Options','sCustom3opt')).strip()
        if mashIni.has_option('Tool Options','sCustom4opt'):
            if not str(mashIni.get('Tool Options','sCustom4opt')).strip() == '-Option(s)':
                inisettings['custom4opt'] = str(mashIni.get('Tool Options','sCustom4opt')).strip()
